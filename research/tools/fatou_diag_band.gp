  ...
  if (efam,
    ...
    if ((ickey == [samples, w, r]) && (type(icct) == "t_POL"),
      ...
      if (icsc > 0,
        ...
        /* DIAG: band structure of the diff poly */
        icthr = 10.0^(-(default(realprecision) + 10));
        ichc = -1; ictc = -1;
        icrl = precision(r, 19);
        icrp = precision(icrl^0, 19);
        for (s = 0, poldegree(icdct),
          if (abs(polcoeff(icdct, s))*icrp > icthr,
            if (ichc < 0, ichc = s);
            ictc = s;
          );
          icrp = icrp*icrl;
        );
        print("DIAG deg=", poldegree(icdct), " band=[", ichc, ",", ictc, "]");
        icfull = 0;
      );
    );
    if (icfull,
